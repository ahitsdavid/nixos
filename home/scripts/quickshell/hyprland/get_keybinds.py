...
def get_keybind_at_line(line_number, line_start = 0):
    ...
    # Check if this is a bindd (bind with description) command
    # bindd can be: bindd, bindde, binddl, binddm, binddr, etc.
    line_stripped = line.lstrip()
    is_bindd = line_stripped.startswith("bindd")
    ...
    # bindd format: mods, key, description, dispatcher, params
    # all other bind formats: mods, key, dispatcher, params
    if is_bindd and len(parts) >= 4:
        mods = parts[0]
        key = parts[1]
        # Skip description (parts[2])
        dispatcher = parts[3]
